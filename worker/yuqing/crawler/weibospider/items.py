...
class TweetItem(Item):
    """Tweet information """
    # _id = Field()  # 微博id
    news_timeStamp = Field()  # 微博发表时间
    news_author = Field()  # 发表该微博用户的id
    news_link = Field()  # 微博URL
    news_comments = Field()         # 评论内容
    news_content = Field()          # 博文
    news_site = Field()
    news_comments_link = Field()      # 新增评论连接
    # 没有comment_title, news_time
    image_url = Field()  # 图片
    video_url = Field()  # 视频
